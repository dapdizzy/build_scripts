...
#COMMON AX FUNCTIONS
############################################################################################
function Synchronize-AX($tableId = 0, $ignoreTimeout = $false)
{
	...
	Write-InfoLog $out
    if (($axProcess.WaitForExit(60000*$SyncTimeout) -eq $false) -and ($ignoreTimeout -eq $false))
	{
...
function Compile-Build
{
    try
    {
        ...
        if ($AxCompileAll -eq "True")   
        {
            ...
            {
                if ([string]::Equals($scriptName, 'BUILD', [System.StringComparison]::OrdinalIgnoreCase))
                {
    ...
    finally
    {
        ...
        if([string]::Equals("$AxCompileAll", 'true', [System.StringComparison]::OrdinalIgnoreCase))
        {
            ...
            if ([string]::Equals("$CompileCIL", 'True', [System.StringComparison]::OrdinalIgnoreCase)) 
            {
                Check-CILErrors
...
function Compile-AX
{
    ...
    #Step 6:
    #Compile-Build
    # Assume it's fine to exit with a Timeout here
    # Comment Synchronization as it seems to be just a waste of time due to compilation Errors
    # Synchronize-AX 0 $true
    Write-InfoLog ("                                                                 ") 
    Write-InfoLog ("                                                                 ") 
    ...
    # After General compilation is finished it's time to try and import VS projects (before that compilation errors might be preventing AX from importing VS projects though).
...
function Get-ModelsToBuild
{
    ...
    Write-Infolog "Going to instect ($LocalProject)"
    $Definition.Load($LocalProject)
    $models = $Definition.SelectSingleNode("//VCSParameters").SelectSingleNode("//Models").ChildNodes
    return $models
}
...
function Combine-Xpos([System.IO.FileSystemInfo]$modelPath)
{
    ...
    Write-Infolog "CmdName = $cmdName, AXBuildDir = $axbuildDir, Arguments = $arguments, logFile = $logFile"
...
{
    Write-Infolog "Creating specific XPOs for file ($xpoFileName)"
    $fileContents = Get-Content $xpoFileName -ErrorAction SilentlyContinue
    $writer = $null
    $shouldDecide = $false
    $buffer = $null
    ...
    foreach ($line in $fileContents)
    {
        ...
        if ($shouldDecide -eq $true -and $line -match '; Microsoft Dynamics AX')
        {
            ...
            elseif ($line -match 'Class: SysImportElements' -or $line -match 'SysStartupCmd' -or $line -match 'Class: SysStartupCmdAOTImport' -or $line -match 'Class: SysStartupCmdCompilePartial' -or $line -match 'MMS_VerifyImportedAOTObjects' -or $line -match 'Class: Global')  # special case for SysImportElements
            {
                if (!$sysClsWriter)
                {
        ...
        if ($line -match 'Element: DBE' -or $line -match 'Element: UT') # All EDTs are User Types in fact, thus UT is the right prefix to look for.
            {
                if (!$baseddWriter)
                {
                    $baseddWriterFileName = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($xpoFileName), ('{0}_basedd.xpo' -f [System.IO.Path]::GetFileNameWithoutExtension($xpoFileName)))
                    $baseddWriter = InitXpoWriter $baseddWriterFileName
                    if (!$writers)
                    {
                        $writers = new-object 'System.Collections.Generic.List[System.IO.StreamWriter]'
                    }
                    $writers.Add($baseddWriter)
                }
                $writer = $baseddWriter
                $writer.WriteLine()
                $copyLine = $true
            }
        elseif ($line -match 'Element: REF')
        {
            if (!$refWriter)
            {
...
# Ensure AOS is Started here
    ...
    try
    {
        do
        {
            $succeeded = Build-VisualStudioProjects ($model)
            if ($succeeded -eq $false)
            {
                # Try to make sure AOS is Started and give it a few more tries just to meke sure it should work according to the logic
                Start-AOS
            }
            $retryCount--
        } while ($succeeded -eq $false -and $retryCount -gt 0)
    }
    catch
    {
        Write-InfoLog ("Building of VS projects failed with error: $(Stringify-Error $error)")
    }
    
    $xpoNamePrefix = ("Combined.{0}" -f $modelName)
    #$xpoName = ("Combined.{0}.xpo" -f $modelName)

    # Seems like at first, we need to import Base Enums, EDTs, Configuration Keys (probably some more) laying under Data Dictionary folder.
    # Otherwise trivial erros complayning that some Enum or EDT cannot be resolved will itch (hurt our ass) while trying to compile SYS-objects (both classes and tables).
    $baseddXpoFileName = gci -Path "$currentLogFolder\*" -Include "*_basedd.xpo" -Name -ErrorAction SilentlyContinue | select -f 1
    if ($baseddXpoFileName -ne $null -and $baseddXpoFileName -ne '')
    {
        Write-InfoLog '------------------------------------------------------------------------------------------------------'
        Write-InfoLog ('---------------Starting to import basic Data Dictionary items *_basedd.xpo at {0}-------------------------' -f (Get-Date))
        
        Import-XPO $baseddXpoFileName
        
        Write-Infolog ('---------------Finished importing of basic Data Dictionary items via Import AOT Startup CMD at {0}-----------------' -f (Get-Date))
        Write-InfoLog '------------------------------------------------------------------------------------------------------'
...
function Stringify-Error($error)
{
    [string]$ret = $null
    $errorEnumerable = $error -as [System.Collections.IEnumerable]
    if ($errorEnumerable -ne $null)
    {
        foreach ($errorEntry in $errorEnumerable)
        {
            if ($ret -ne $null)
            {
                $ret += ', '
            }
            $ret += $errorEntry.ToString()
        }
    }
    else
    {
...
{
    # Make it a script scoped variable to access it later in another function
    $script:assembliesHashSet = new-object System.Collections.Generic.HashSet[String]
    ...
    foreach ($refsFileName in (gci -Path "$currentLogFolder\*" -Include "*_refs.xpo" -Name))
    {
        ...
        { 
            if ($line -match '\b(?<assemblyname>MMS[\.\wa-zA-Z0-9._%+-]+)[ ,]+Version\=(?<version>\d+(?:\.\d+)+)\b')
            {
...
function Install-Packages
{
    ...
    foreach ($assemblyInfo in $script:assembliesHashSet)
    {
        $split = $assemblyInfo.Split([char[]]':')
        $newDir = (Join-Path (Join-Path $currentLogFolder Packages) $split[0])
        New-Item $newDir -ItemType Directory -Force
        $nugetInstallExpr = ('nuget install "{0}" -Version {1} -o "{2}"' -f $split[0], $split[1], $newDir)
...
function Clean-Build
{
    ...
    Install-ModelStore (join-path $backupModelStoreFolder "WAXR3_baseline1.axmodelstore")
    Write-InfoLog ('---------------------------------------------------------------')
    ...
    Write-Infolog "Skipping restore for now"
